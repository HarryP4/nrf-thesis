...
static K_SEM_DEFINE(sem_remoteinfo, 0, 1);
...
static void request_cb(struct bt_le_ext_adv *adv, const struct bt_le_per_adv_data_request *request)
{
	...
	//	printk("Subevent data set %d - subevents: %d, counts %d \n", counter,request->start,request->count);
	}
}
...
static bool print_ad_field(struct bt_data *data, void *user_data)
{
	...
	printk("Temperature of the node: %d ",data->data[2]/4);
	printk("\n");
...
{
	if (buf) {
		printk("Response: subevent %d, response slot %d ", info->subevent, info->response_slot);
...
int main(void)
{
	...
	while (num_synced < MAX_SYNCS) {
		...
		if (k_sem_take(&sem_remoteinfo,K_MSEC(1000))!=0)
		{
			printk("Failed to get remote info or link is broken, disconnect");
			goto disconnect;
		}
